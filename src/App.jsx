  ...
  import { 
    Play, Pause, Mic, Music, Drum, Guitar, Layers, 
    Settings, MoreVertical, Wand2, Plus, Download, X, Edit3, 
    Scissors, Sparkles, Undo, Save, UploadCloud, 
    FileAudio, Smartphone, ArrowLeft, Trash2, Zap, Lock, 
    LogOut
  } from 'lucide-react';

  // --- FIREBASE IMPORTS (Optional for Mock) ---
  ...
  const APP_CONFIG = {
    name: "SurAI",
    version: "1.3.1 (Audio Fix)", // Version Bump
    vipEmails: [
      "admin@surai.com", 
      "review@test.com",
      "friend@gmail.com"
    ],
    // Demo Audio for UI Testing (Lo-fi beat sample)
    demoAudioUrl: "https://cdn.pixabay.com/download/audio/2022/05/27/audio_1808fbf07a.mp3?filename=lofi-study-112191.mp3"
  };

  // Initialize Firebase (Safe Mode)
  let auth;
  try {
    if (FIREBASE_CONFIG.apiKey && FIREBASE_CONFIG.apiKey !== "AIzaSy...") {
  ...
  const TRACK_TYPES = [
    ...
    { type: 'piano', name: 'Piano', icon: Music, color: 'bg-purple-500' }, // Fixed Icon
  ...
  export default function SurAIApp() {
    ...
    // 🔊 AUDIO ENGINE (Mock Player - Fixed)
    const audioRef = useRef(new Audio(APP_CONFIG.demoAudioUrl));
    ...
    useEffect(() => {
      ...
      // Play/Pause Logic with Interruption Handling
      if (isPlaying && view === 'STUDIO') {
        const playPromise = audioRef.current.play();
        
        // Handle the race condition where pause() is called before play() finishes
        if (playPromise !== undefined) {
          playPromise.catch(error => {
            // If error is just "interrupted", ignore it. Otherwise log it.
    ...
    const handleGoogleLogin = async () => {
      // Mock Login (Always works)
      if (!auth) {
        const mockUser = {
          email: "admin@surai.com",
          photo: null,
          isPremium: true
        };
        setUser(mockUser);
          ...
          <Modal onClose={() => setShowLyricModal(false)} title="Lyric Extension (SVS)">
            ...
            <div className="flex flex-wrap gap-2 justify-between items-center">
              <div className="flex gap-2">
                  ...
                  <button onClick={() => callGemini('suggest_chords')} className="text-xs text-blue-400 hover:text-white flex items-center gap-1 border border-gray-700 px-2 py-1 rounded-full"><Music className="w-3 h-3"/> {isAiThinking ? 'Analyzing...' : 'Get Chords'}</button>
              </div>
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6 relative overflow-hidden">
        ...
        <div className="bg-gray-900/80 backdrop-blur-xl border border-gray-800 p-8 rounded-3xl w-full max-w-md shadow-2xl z-10 text-center">
          ...
          >
            {/* Using a simpler icon for reliability */}
            <div className="w-5 h-5 bg-black rounded-full flex items-center justify-center text-white text-[10px] font-bold">G</div>
            Sign in with Google
          </button>
          <div className="mt-6 text-xs text-gray-500">
            Config Missing? It will fallback to Mock Mode.
          </div>
        </div>
      </div>
    );
  ...
  // ... (Rest of Sub-Components: HomeScreen, HomeCard, GenerateSetup, RemixSetup, ProcessingScreen, Modal, TimelineRuler, MockWaveform, TrackControl are same as before) ...
